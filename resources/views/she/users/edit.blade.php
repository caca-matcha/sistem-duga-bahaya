                    ...
                    <div
                        ...
                        <div>
                            <div
                                ...
                                <h3 class="text-xs font-black text-gray-500 uppercase tracking-[0.2em]">Data Personel
                                </h3>
                            </div>
                            <p class="text-sm text-gray-400 font-medium">Perbarui detail identitas dan jabatan untuk:
                                <span class="font-semibold text-red-600">{{ $user->name }}</span>
                            </p>
                        </div>
                    </div>

                    <form action="{{ route('she.users.update', $user->id) }}" method="POST" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" name="organization_unit" id="organization_unit"
                                    class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500 sm:text-sm transition duration-150"
                                    value="{{ old('organization_unit', $user->organization_unit) }}">
                            </div>
                            <div>
                                ...
                                <input type="text" name="job_family" id="job_family"
                                    class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500 sm:text-sm transition duration-150"
                                    value="{{ old('job_family', $user->job_family) }}">
                            </div>
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6 border-t border-gray-100">
                            <div>
                                <label for="password" class="block text-sm font-semibold text-gray-700 mb-1">New
                                    Password
                                    (Optional)</label>
                            ...
                            <div>
                                <label for="password_confirmation"
                                    class="block text-sm font-semibold text-gray-700 mb-1">Confirm New Password</label>
                        ...
                        <div>
                            ...
                            <select name="role" id="role"
                                class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500 sm:text-sm transition duration-150"
                                required>
                                <option value="karyawan" @selected(old('role', $user->role) == 'karyawan')>Karyawan
                                </option>
                                <option value="supervisor" @selected(old('role', $user->role) == 'supervisor')>Supervisor
                                </option>
                                <option value="she" @selected(old('role', $user->role) == 'she')>SHE (Admin)</option>
                                <option value="magang" @selected(old('role', $user->role) == 'magang')>Magang</option>
                            </select>
                            @error('role')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
