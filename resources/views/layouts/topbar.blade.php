...
<header x-data="{ showSheChatModal: false}"
    ...
    <div class="flex items-center gap-2">
        <h1 class="text-xl md:text-2xl font-black text-red-600 tracking-tighter"
            style="font-family: 'Inter', sans-serif;">SIDUBA</h1>
        <span
            class="hidden sm:inline text-[10px] md:text-xs text-gray-400 font-medium uppercase tracking-widest px-2 py-0.5 bg-gray-50 rounded-full border border-gray-100">Sistem
            Duga Bahaya</span>
    </div>
        ...
        <div class="flex items-center space-x-1 md:space-x-2">
            ...
            <div class="relative">
                <button id="notification-bell"
                    ...
                    title="Notifikasi">
                    <svg class="h-5 w-5 md:h-6 md:w-6" xmlns="http://www.w3.org/2000/svg" fill="none"
                ...
                <div id="notification-modal"
                    class="absolute right-0 mt-3 w-80 max-w-[calc(100vw-2rem)] bg-white rounded-lg shadow-2xl overflow-hidden border border-gray-100 opacity-0 scale-95 pointer-events-none transition duration-200 origin-top-right z-50">

                    <div
                        class="p-4 border-b border-gray-100 flex justify-between items-center bg-white sticky top-0 z-10">
                        <h3 class="text-lg font-bold text-gray-800">Notifikasi Terbaru</h3>
                        <span id="notification-modal-count"
                            class="text-xs font-semibold text-purple-600 bg-purple-50 px-2 py-0.5 rounded-full"
                            style="display: none;">0 Baru</span>
                    </div>

                    <div id="notification-list" class="max-h-96 overflow-y-auto overscroll-contain">
                    ...
                    @if (Auth::check() && Auth::user()->hasRole('karyawan'))
                        <div class="p-2 bg-gray-50 text-center border-t border-gray-100">
                            <button id="mark-all-read-btn"
                                class="text-sm font-semibold text-indigo-600 hover:text-indigo-800 transition duration-150 w-full">
        ...
        function fetchSheNotifications() {
            ...
            axios.get('{{ route('she.api.hazards.notifications') }}')
                .then(response => {
                    ...
                    const notificationsToDisplay = notifications; // Tampilkan semua
        ...
        function fetchKaryawanNotifications() {
            ...
            axios.get("{{ route('karyawan.notifications.index') }}")
                .then(response => {
                    ...
                    const notificationsToDisplay = allNotifications; // Tampilkan semua
